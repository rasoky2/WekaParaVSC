import { IWekaMetrics, IDatasetStats, IAttributeStats } from '../interfaces/weka.interface';
import { DatasetRecommendationService } from './datasetRecommendationService';

export class StatisticsService {
    private readonly recommendationService: DatasetRecommendationService;

    constructor() {
        this.recommendationService = new DatasetRecommendationService();
    }

    public parseWekaOutput(output: string): IWekaMetrics {
        ...
        const modelInfo: IWekaMetrics['modelInfo'] = {};
        ...
        if (leavesMatch) modelInfo.numberOfLeaves = Number(leavesMatch[1]);
        if (treeSizeMatch) modelInfo.sizeOfTree = Number(treeSizeMatch[1]);
        if (rulesMatch) modelInfo.numberOfRules = Number(rulesMatch[1]);
        ...
        const classDetails: IWekaMetrics['classDetails'] = [];
    ...
    private formatDetailedOutput(output: string): string {
    ...
    private parseBasicStats(lines: string[]): Partial<IDatasetStats> {
        const stats: Partial<IDatasetStats> = {
            numInstances: 0,
            numAttributes: 0,
            numNumerical: 0,
            numNominal: 0,
    ...
    private parseAttributeInfo(line: string): IAttributeStats | null {
    ...
    private updateAttributeStats(
        currentAttribute: IAttributeStats,
        line: string,
        stats: IDatasetStats
    ): void {
    ...
    public parseDatasetStats(output: string): IDatasetStats {
        const lines = output.split('\n');
        const stats = this.parseBasicStats(lines) as IDatasetStats;
        let currentAttribute: IAttributeStats | null = null;
    ...
    private calculateCorrelations(attributes: IAttributeStats[]): number[][] {
    ...
    private getDefaultStats(): IDatasetStats {
        return {
            numInstances: 0,
            numAttributes: 0,
            numNumerical: 0,
